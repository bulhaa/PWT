...
apiTest_view(){
	...
	t := "    /**`n     * can view ? valueS1 ?`n     *`n     * @test`n     * @group now`n     */`n    public function can_view_? valueS1 ?()`: void`n    {`n        $? valueS1 ? = ? valueCC1 ?`:`:factory()->create()`;`n        $user = User`:`:factory()->create()`;`n        $this->actingAs($user)->havingPermission('? valueS2 ?.show')`;`n        $response = $this->getJson('/? valueS2 ?/'.$? valueS1 ?->id)`;`n        $response->assertOk()->assertJson([`n            'data' => [`n" data "`n            ]`,`n        ])`;`n    }`n"
